...
class GithubTelegramBotClass
{
    ...
    {
        static::boot();
        $this->catchTelegramData()->resolveTelegramData();
        ...
        if (empty($this->chatId)) {
            ...
            if (in_array($this->chatId, ADMINS)) {
                $this->sendTelegram($this->message_body);
            } else {
                $this->accessDenied();
            }
        }
    ...
    public function catchTelegramData()
    {
        $this->request = Request::createFromGlobals();
        $this->telegram = new Telegram(BOT_API_TOKEN);
    ...
    public function resolveTelegramData()
    {
        $this->message_body = $this->result['message'] ['text'];
        $this->chatId = $this->result['message'] ['chat']['id'];
        $this->first_name = $this->telegram->FirstName();
     ...
     */
    public function sendMessage($admin): bool
    {
        $this->getPayload();
        $text = $this->charReplace();
        $method_url = 'https://api.telegram.org/bot'.BOT_API_TOKEN.'/sendMessage';
